...
class list
{
  ...
  public:
    ...
    void reorder(int pos,std::vector<int> selected) {
      ...
      it =  begin();
      count = 0;
      while((i < selected.size())&&(count <= mSize)){
        if(count == selected[i]){
          auto curNode = it;
          it++;
          curNode.ptr->prev->next = curNode.ptr->next;
          curNode.ptr->next->prev = curNode.ptr->prev;
          curNode.ptr->prev = ins.ptr->prev;
          curNode.ptr->next = ins.ptr;
          ins.ptr->prev->next = curNode.ptr;
          ins.ptr->prev = curNode.ptr;
          i++;
        }else{
          it++;
        }
