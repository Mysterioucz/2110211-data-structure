...
class priority_queue
{
  ...
  public:
    ...
    void erase(const T& v) {
      ...
      int idx = 0;
      while((idx < mSize) && mData[idx] != v){ //หา index ของตัวที่ต้องการลบ
        idx++;
      }
      if(idx != mSize){
        int leaf = idx; 
        while(2*leaf + 1 < mSize){ // หา index ของ leaf อันสุดท้ายของ binary heap ย่อยนี้โดยไล่ลงไปด้วย Left Child เรื่อยๆ
          leaf = 2*leaf + 1;
        }
        if(leaf + 1 < mSize){ // check ว่ามี leaf ด้านขวาไหม
          leaf++;
        }
        mData[idx] = mData[leaf]; //ย้ายข้อมูลขึ้นมา
        mSize--;
        fixDown(idx); // เรียง Binary Heap ย่อยนี้ใหม่
        while(leaf < mSize){ // เลื่อน Leaf
          mData[leaf] = mData[leaf+1];
          fixUp(leaf); // แก้ลำดับ Leaf ในการณีที่ node นี้มาจาก sub heap อื่น
          leaf++;
        }
      }
    }
