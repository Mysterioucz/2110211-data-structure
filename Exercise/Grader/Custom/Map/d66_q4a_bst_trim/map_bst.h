...
class map_bst
{
  ...
  public:
    ...
    void setAbleToCreateNode(bool value, std::string key) {
		if(key == "523252TTelgniSsIrettoPyhW")
			ableToCreateNode = value;
		else
			throw std::out_of_range("wrong key");
	  }

	  bool getAbleToCreateNode() {
		  return ableToCreateNode;
	  }

    int getDeleteNodeCount() {
